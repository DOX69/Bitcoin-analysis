"""Adaptive exploratory causal quantile stacking, never independent confirmation."""
...
def predict(closes, origin, student_quantiles):
    ...
    for h in range(1, 53):
        ...
        observed = known[indices + h]
        horizon_points, horizon_weights = [], []
        for qi, q in enumerate(b.QUANTILES):
            gauss = p_gaussian[indices - 104, h - 1, qi]
            student = p_student[indices - 104, h - 1, qi]
            losses = []
            for weight in WEIGHTS:
                error = observed - (weight * student + (1 - weight) * gauss)
                losses.append(float(np.mean(np.maximum(q * error, (q - 1) * error))))
            weight = choose_weight(losses)
            horizon_weights.append(weight)
            horizon_points.append(
                float(
                    weight * p_student[-1, h - 1, qi]
                    + (1 - weight) * p_gaussian[-1, h - 1, qi]
                )
            )
        horizon_points[2] = closes[origin]
        points.append(sorted(horizon_points))
        weights.append(horizon_weights)
        calibration.append(
            {
                "first_origin": first,
                "last_origin": last,
...
def prepare(snapshot, directory):
    ...
    write_json(
        directory / "manifest.json",
        {
            ...
            "recipe": "causal_quantile_stacking_gaussian104_student5_drift104",
            ...
            "weights": WEIGHTS,
            "algorithm": "Each origin t,h,q: latest156 mature origins j>=104,j+h<=t. Causal sigma104 at each j for Gaussian and frozen Student trajectory quantiles. Minimize mean USD pinball over weights0,.25,.5,.75,1. Tie nearest.5 then lower. Price quantile w*Student+(1-w)*Gaussian; median P[t]; sort5quantiles. No manual horizon adjustments.",
