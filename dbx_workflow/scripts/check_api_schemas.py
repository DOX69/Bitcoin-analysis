...
def generate_model(input_data, output_file, class_name, input_type="json"):
    """
    Generates Pydantic model from input data (JSON or JSON Schema).
...
def main():
    ...
    # Prepare JSON Schema using Genson
    if "rates" in f_data and isinstance(f_data["rates"], dict) and f_data["rates"]:
        # 1. Generate schema for the 'rate' object (inner value)
        first_val = next(iter(f_data["rates"].values()))
        rate_builder = SchemaBuilder()
        rate_builder.add_object(first_val)
        rate_schema = rate_builder.to_schema()

        # 2. Generate schema for the main object (excluding rates to avoid dynamic keys polluting it)
        data_without_rates = {k: v for k, v in f_data.items() if k != "rates"}
        main_builder = SchemaBuilder()
        main_builder.add_object(data_without_rates)
        full_schema = main_builder.to_schema()

        # 3. Inject 'rates' with additionalProperties
        # We manually define the 'rates' property to map string keys to the rate_schema
        full_schema['properties']['rates'] = {
            "type": "object",
            "additionalProperties": rate_schema
        }

        # Ensure 'rates' is required
        ...
        logger.warning("Frankfurter response did not contain expected 'rates' data. Skipping schema update.")
