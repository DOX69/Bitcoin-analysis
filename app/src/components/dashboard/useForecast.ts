...
const point = z.object({ horizonWeeks: z.number().int().min(1).max(52).optional(), horizonDays: z.number().int().min(1).max(365).optional(), targetDate: date, q25: z.number().positive(), q50: z.number().positive(), q75: z.number().positive() }).refine(p => p.q25 <= p.q50 && p.q50 <= p.q75);
...
}).refine(value => value.emissions.every(emission => emission.status === 'invalidated' || (emission.points.length === (value.frequency === 'daily' ? 365 : 52) && emission.points.every((p, i) => (value.frequency === 'daily' ? p.horizonDays === i + 1 && p.horizonWeeks === undefined : p.horizonWeeks === i + 1 && p.horizonDays === undefined) && Date.parse(p.targetDate) === Date.parse(emission.originDate) + (i + 1) * (value.frequency === 'daily' ? 1 : 7) * 86400000))));
...
export function useForecast(enabled: boolean, currency: Currency, start: string, end: string) {
    ...
    return { status: current?.status ?? (enabled ? 'loading' : 'absent'), model: response?.model ?? null, frequency: response?.frequency ?? 'weekly', emissions, projection };
}
