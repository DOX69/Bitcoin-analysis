...
import PriceFreshness from '@/components/dashboard/PriceFreshness';
            ...
            <Card className="border-0 bg-transparent p-0 shadow-none ring-0">
                <CardHeader className="gap-3 p-0">
                    <div className="flex items-center justify-between gap-3">
                        ...
                        <Badge variant="outline">Daily candle</Badge>
                    </div>
                    <PriceFreshness observedAt={metrics?.observedAt} dataAgeDays={metrics?.dataAgeDays} />
                    <p className="text-xs text-muted-foreground">Price: Coinbase · FX: Frankfurter</p>
                </CardHeader>

                <CardContent className="p-0 pt-6">
                    <dl className="grid grid-cols-2 gap-x-4 gap-y-6">
                        {[
                            ['Latest observed price', metrics ? formatCurrency(metrics.currentPrice, currencySymbol) : 'Unavailable'],
                            ...
                            ['RSI (14d)', metrics?.rsi != null ? metrics.rsi.toFixed(1) : 'Unavailable'],
                        ].map(([label, value]) => (
                            <div key={label}>
